...
QUAL_MAP = {"T_degC": "T_qual", "O2ml_L": "O_qual"}  # ChlorA has no qual column
...
def load_and_join():
    ...
    cast = pd.read_csv(
        CAST_FILE,
        usecols=["Cst_Cnt", "Sta_ID", "Year", "Lat_Dec", "Lon_Dec"],
        low_memory=False,
    )
...
def aggregate_annual(df, eligible_stations):
    df = df[df["Sta_ID"].isin(eligible_stations)]

    station_coords = (
        df.groupby("Sta_ID")[["Lat_Dec", "Lon_Dec"]].median().reset_index()
    )

    annual = (
        df.groupby(["Sta_ID", "Year"])[VARS]
        .mean()
        .reset_index()
...
def compute_baseline_stats(annual):
    baseline = annual[annual["Year"] <= BASELINE_END].copy()
    baseline["log_ChlorA"] = np.log(baseline["ChlorA"].clip(lower=1e-4))

    stats = (
        baseline.groupby("Sta_ID")[["T_degC", "O2ml_L", "log_ChlorA"]]
        .agg(["mean", "std"])
        .reset_index()
    )
...
def compute_scores(annual, baseline_stats):
    scoring = annual[annual["Year"] >= SCORING_START].copy()
    scoring["log_ChlorA"] = np.log(scoring["ChlorA"].clip(lower=1e-4))
    scoring = scoring.merge(baseline_stats, on="Sta_ID")
...
def save_outputs(scores, annual, baseline_stats):
...
if __name__ == "__main__":
    ...
    print("Aggregating to annual means per station...")
    annual = aggregate_annual(df, eligible)

    print("Computing baseline stats (1949–1990)...")
    baseline_stats = compute_baseline_stats(annual)

    print(f"Scoring {SCORING_START}–present...")
    ...
    save_outputs(scores, annual, baseline_stats)
